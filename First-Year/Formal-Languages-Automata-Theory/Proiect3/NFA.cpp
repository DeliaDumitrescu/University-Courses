...
void NFA::RegularGrammarToNFA()
{
    std::map < char, std::vector<std::string> > grammar;
    std::map < char, std::vector<std::string> > epsFreeGrammar;
    std::vector <char> eps;                    //starile care contin #
    ...
    for(i = 0; i < x ; i++)
    {
        ...
        if(pr == "#")
            ...
            grammar[ch].push_back(pr);
    }

    std::map<char, std::vector<std::string>>::iterator it;
    for(it = grammar.begin(); it != grammar.end(); it++)
    {
        ch = it->first; //pt fiecare S: {...}, A:{}, B:{}

        if(states.find(it->first) == states.end())
            states[it->first] = ind++;

        std::vector<std::string> prods = it->second;
        epsFreeGrammar[ch] = prods;
        for(int i = 0; i < prods.size(); i++)
            for(int e = 0; e < eps.size(); e++)
            {

                int poz = prods[i].find(eps[e]);
                std::string p = prods[i];
                if(poz != std::string::npos)
                {
                    p.erase(poz);
                    if(std::find(epsFreeGrammar[ch].begin(), epsFreeGrammar[ch].end(), p) == epsFreeGrammar[ch].end())
                        epsFreeGrammar[ch].push_back(p);
                }
            }
    }

    if(std::find(eps.begin(), eps.end(), 'S') != eps.end())
    {   epsFreeGrammar['Z'] = epsFreeGrammar['S'];                //Z e echiv lui S1
        epsFreeGrammar['Z'].push_back("#");
        states['Z'] = ind++;
    }
    states['D'] = ind++;

    for(it = epsFreeGrammar.begin(); it != epsFreeGrammar.end(); it++)
        for(int i =0; i < it->second.size(); i++)
            if(it->second[i].size() == 1)            // pt cele dintr-un singur caracter gen A a
            {
                if(it->second[i] != "#")
                {
                    std::cout << it->first << " " << 'D' << " " << it->second[i] << '\n';
                    a[states[it->first]].push_back({states['D'],*(it->second[i].c_str())});
                    m++;
                }
            }
            else     //pt cele de forma A aA (2caractere)
            {   std:: cout << it->first << " " << it->second[i][1] << " " << it->second[i][0] << '\n';
                a[states[it->first]].push_back({ states[it->second[i][1]], it->second[i][0] });
                m++;
                if(states[it->second[i][1]] == 0 && grammar.find(it->second[i][1]) == grammar.end() )  //pt nodurile care apar doar in productii
                    {  states[it->second[i][1]] = ind++;
                        std::cout<<ind;
                    }
            }


     if(epsFreeGrammar.find('Z')!= epsFreeGrammar.end())
    {
        qf[states['Z']] = 1;
        q0 = states['Z'];
        qi[states['Z']] = 1;
    ...
    std::map <char,int>::iterator ip;
    for(ip = states.begin(); ip != states.end(); ip++)
